...
install_package_list() {
  ...
  mapfile -t INCLUDED_PACKAGES < <(jq -r "[(.all.include | (select(.\"$PACKAGE_LIST\" != null).\"$PACKAGE_LIST\")[]), \
  (select(.\"$FEDORA_MAJOR_VERSION\" != null).\"$FEDORA_MAJOR_VERSION\".include | (select(.\"$PACKAGE_LIST\" != null).\"$PACKAGE_LIST\")[])] \
  | sort | unique[]" /tmp/packages.json)

  # build list of all packages requested for exclusion
  mapfile -t EXCLUDED_PACKAGES < <(jq -r "[(.all.exclude | (select(.\"$PACKAGE_LIST\" != null).\"$PACKAGE_LIST\")[]), \
...
install_package_list "orora"
install_package_list "orora-dx"
