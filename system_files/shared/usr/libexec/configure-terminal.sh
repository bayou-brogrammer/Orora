...
function Exiting() {
    ...
    echo "Rerun CLI setup using ${blue}ujust orora-cli${normal}..."
    exit 0
}
function Good_Exit() {
    echo ""
    echo "Finished Orora-CLI setup, rerun with ${blue}ujust orora-cli${normal} to reconfigure"
    exit 0
}
...
#         cp "/usr/share/ublue-os/orora-cli/${2}.sh" "${HOME}/.bashrc.d/00-container.sh"
#     else
